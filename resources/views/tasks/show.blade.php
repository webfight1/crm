                ...
                <div class="p-6 text-gray-900">
                    <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <dd class="mt-1 space-y-2">
                                @forelse($task->timeEntries as $timeEntry)
                                    <div x-data="{ editingNotes: false }" class="text-sm text-gray-900 bg-gray-50 p-2 rounded">
                                        <div class="flex items-center justify-between">
                                            ...
                                            @if($timeEntry->end_time)
                                                <div class="flex items-center space-x-2">
                                                    <button type="button" @click="editingNotes = !editingNotes"
                                                            class="text-gray-500 hover:text-gray-800 text-xs"
                                                            title="{{ $timeEntry->notes ? 'Muuda kommentaari' : 'Lisa kommentaar' }}">
                                                        💬
                                                    </button>
                                        ...
                                        {{-- Notes: shown inline when present, otherwise revealed by 💬 button.
                                             Toggling editingNotes opens a small inline form that PATCHes notes only. --}}
                                        @if($timeEntry->notes && !old('editing-' . $timeEntry->id))
                                            <p class="mt-1 ml-7 text-xs text-gray-600 italic"
                                               x-show="!editingNotes">{{ $timeEntry->notes }}</p>
                                        @endif

                                        @if($timeEntry->end_time)
                                            <form x-show="editingNotes" x-cloak
                                                  method="POST" action="{{ route('time-entries.notes', $timeEntry) }}"
                                                  class="mt-2 ml-7 space-y-1.5">
                                                @csrf
                                                @method('PATCH')
                                                <textarea name="notes" rows="2"
                                                          placeholder="Mida selle aja jooksul tegid?"
                                                          class="block w-full text-xs rounded border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">{{ $timeEntry->notes }}</textarea>
                                                <div class="flex justify-end gap-2">
                                                    <button type="button" @click="editingNotes = false"
                                                            class="text-xs text-gray-500 hover:text-gray-700">Tühista</button>
                                                    <button type="submit"
                                                            class="text-xs bg-indigo-600 hover:bg-indigo-700 text-white px-2 py-1 rounded">Salvesta</button>
                                                </div>
                                            </form>
                                        @endif
                                    </div>
                        ...
                        <div>
                            ...
                            <dd class="mt-1">
                                @if($task->work_type === 'technical')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-indigo-100 text-indigo-800">Tehniline</span>
                                @elseif($task->work_type === 'design')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-pink-100 text-pink-800">Disain</span>
                                @elseif($task->work_type === 'copywriting')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-teal-100 text-teal-800">Tekstid</span>
                                @elseif($task->work_type === 'marketing')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-purple-100 text-purple-800">Turundus</span>
                                @elseif($task->work_type === 'ecommerce')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-orange-100 text-orange-800">E-kaubandus</span>
                                @elseif($task->work_type === 'website')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-cyan-100 text-cyan-800">Veebileht</span>
                                @elseif($task->work_type === 'project')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">Projekt</span>
                                @elseif($task->work_type === 'maintenance')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Hooldus</span>
                                @else
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Muu</span>
                                @endif
                            </dd>
                        </div>
                        @endif

                        @if($task->clarity_level)
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Selgus</dt>
                            <dd class="mt-1">
                                @if($task->clarity_level === 'clear')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">✅ Selge</span>
                                @elseif($task->clarity_level === 'medium')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Keskmine</span>
                                @elseif($task->clarity_level === 'vague')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">❓ Ebaselge</span>
                                @endif
                            </dd>
                        </div>
                        @endif

                        @if($task->revenue_model)
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Tulu mudel</dt>
                            <dd class="mt-1">
                                @if($task->revenue_model === 'hourly_partner')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">🔥 Tunnitasu partner</span>
                                @elseif($task->revenue_model === 'retainer')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">Püsiklient</span>
                                @elseif($task->revenue_model === 'fixed_project')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-purple-100 text-purple-800">Fikseeritud projekt</span>
                                @elseif($task->revenue_model === 'internal')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Sisemine</span>
                                @elseif($task->revenue_model === 'uncertain')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Ebakindel</span>
                                @endif
                            </dd>
                        </div>
                        @endif

                        @if($task->cashflow_speed)
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Raha kiirus</dt>
                            <dd class="mt-1">
                                @if($task->cashflow_speed === 'fast')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">⚡ Kiire</span>
                                @elseif($task->cashflow_speed === 'medium')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Keskmine</span>
                                @elseif($task->cashflow_speed === 'slow')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">🐌 Aeglane</span>
                                @endif
                            </dd>
                        </div>
                        @endif
                        ...
                        <div>
                            ...
                            <dd class="mt-1">
                                @if($task->risk_level === 'low')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Madal</span>
                                @elseif($task->risk_level === 'medium')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Keskmine</span>
                                @elseif($task->risk_level === 'high')
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">⚠️ Kõrge</span>
                                @endif
                            </dd>
                        </div>
                        @endif
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900">{{ $task->estimated_hours }} tundi</dd>
                        </div>
                        @endif

                        @if($task->value_score)
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Väärtuse skoor</dt>
                            <dd class="mt-1">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    @if($task->value_score >= 8) bg-green-100 text-green-800
                                    @elseif($task->value_score >= 5) bg-yellow-100 text-yellow-800
                        ...
                        @if($task->cashflow_score)
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Rahavoo skoor</dt>
                            <dd class="mt-1">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    @if($task->cashflow_score >= 8) bg-green-100 text-green-800
                                    @elseif($task->cashflow_score >= 5) bg-yellow-100 text-yellow-800
                                    @else bg-red-100 text-red-800 @endif">
                                    {{ $task->cashflow_score }}/10
                                </span>
                            </dd>
                        </div>
                        @endif
                        ...
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Kiire võit</dt>
                            <dd class="mt-1">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">⚡ Jah</span>
                            </dd>
                        </div>
                        @endif
                        ...
                        @if($task->is_blocking)
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Blokeeriv</dt>
                            <dd class="mt-1">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">🚫 Jah</span>
                            </dd>
                        </div>
                        @endif

                        @if($task->recommended_next_step)
                        <div class="md:col-span-2">
                            <dt class="text-sm font-medium text-gray-500">Soovitatud järgmine samm</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $task->recommended_next_step }}</dd>
                        </div>
                        @endif
